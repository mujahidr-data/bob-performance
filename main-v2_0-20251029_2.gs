...
function pullFields() {
  ...
  if (resp.getResponseCode() < 200 || resp.getResponseCode() >= 300) {
    throw new Error(`Fields fetch failed (${resp.getResponseCode()}): ${resp.getContentText().slice(0,300)}`);
  }
...
function setupAndPullEmployees() {
  ...
  // Row 6: Tip
  sh.getRange('A6').setValue('💡 TIP: Use "All" status to include future hires & terminated employees')
    .setFontStyle('italic')
    .setFontColor('#E67C73')
...
function pullEmployeesData_(sh, auth) {
  ...
  if (resp.getResponseCode() !== 200) {
    throw new Error(`Failed to fetch employees (${resp.getResponseCode()}): ${resp.getContentText().slice(0,300)}`);
  }
  ...
  // Freeze rows and create filter starting from row 7
  sh.setFrozenRows(7);
